...
void DCPPEpEm_factory::Process(const std::shared_ptr<const JEvent>& event)
{
  ...
  if (VetoNeutrals(t0_rf,vertex,neutrals)==false){
	...
	if ((*locParticleIterator)->Get_KinFitParticleType()==d_DetectedParticle){
      ...
      if( PiMuFillFeatures(event, piplus, piminus, pimu_input) ){
...
// and 1 pi- candidate.
bool DCPPEpEm_factory::PiMuFillFeatures(const std::shared_ptr<const JEvent>& event, const DTrackTimeBased *piplus, const DTrackTimeBased *piminus, float *features){
  
  vector<const DFMWPCMatchedTrack*> matchedtracks;
  event->Get( matchedtracks );
