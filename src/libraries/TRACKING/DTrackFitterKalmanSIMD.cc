...
inline bool static DKalmanSIMDFDCHit_cmp(DKalmanSIMDFDCHit_t *a, DKalmanSIMDFDCHit_t *b){
  if (fabs(a->z-b->z)<EPS){
    if (a->hit!=NULL && b->hit!=NULL && fabs(a->t-b->t)<EPS){
...
//  Add CDC hits
void DTrackFitterKalmanSIMD::AddCDCHit (const DCDCTrackHit *cdchit){
...
jerror_t DTrackFitterKalmanSIMD::SetCDCForwardReferenceTrajectory(DMatrix5x1 &S){
   ...
   while(z<endplate_z_downstream && z>cdc_origin[2] &&
...
jerror_t DTrackFitterKalmanSIMD::KalmanLoop(void){
   ...
   if (fit_type==kWireBased && theta_deg>10.){
     ...
     if (my_fdchits.size()>0 && my_fdchits[0]->hit!=NULL){
	...
	if (chi2check<chi2cut){	  
	  ...
	  if (DEBUG_LEVEL>9)
	    printf("Ring %d straw %d pred %f meas %f chi2 %f useBrent %i \n",
		   ...
		   d,dm,(1.-H*K)*res*res/V,swimmed_to_doca);
...
shared_ptr<TMatrixFSym> DTrackFitterKalmanSIMD::Get7x7ErrorMatrix(DMatrixDSym C){
   ...
   *C7x7=C.Similarity(J);
   // C7x7->Print();
   //   _DBG_ << " " << C7x7->operator()(3,3) << " " << C7x7->operator()(4,4) << endl;
...
kalman_error_t DTrackFitterKalmanSIMD::RecoverBrokenTracks(double anneal_factor,
   ...
   while (break_point_cdc_index>4 && break_point_step_index>0 
      ...
      refit_ndf=0;
      // Now refit with the truncated trajectory and list of hits
      refit_error=KalmanForwardCDC(anneal_factor,S1,C1,refit_chisq,refit_ndf);
      if (refit_error==FIT_SUCCEEDED 
            || (refit_error==BREAK_POINT_FOUND
               && break_point_cdc_index==1
               )
...
kalman_error_t DTrackFitterKalmanSIMD::ForwardFit(const DMatrix5x1 &S0,const DMatrix5x5 &C0){   
   ...
   kalman_error_t error=FIT_NOT_DONE,last_error=FIT_NOT_DONE;
   ...
   for (unsigned int m=0;m<last_fdc_used_in_fit.size();m++){
      if (last_fdc_used_in_fit[m] && my_fdchits[m]->hit!=NULL){
      ...
      if (forward_traj[m].h_id>0){
		 ...
		 + my_fdchits[id]->phiX;
		   ...
		   && my_fdchits[id]->hit->wire->layer==PLANE_TO_SKIP){
               ...
               if (ALIGNMENT_FORWARD && my_fdchits[id]->hit!=NULL){
               ...
               if (DEBUG_LEVEL>19 && my_fdchits[id]->hit!=NULL){
...
jerror_t DTrackFitterKalmanSIMD::FillPullsVectorEntry(const DMatrix5x1 &Ss,
   ...
   // wrt the wire origin, direction, and the track parameters.
   vector<double> alignmentDerivatives;
   if (ALIGNMENT_FORWARD && hit->hit!=NULL){
...
// Find extrapolations to detector layers within the tracking volume and 
// inward (toward the target).
jerror_t DTrackFitterKalmanSIMD::ExtrapolateToInnerDetectors(){
  ...
  //--------------------------------------------------------------------------
  // The following code continues the extrapolations to wire planes that were
  // not included in the forward trajectory
  //--------------------------------------------------------------------------
  ...
  // Find intersection points to FDC planes beyond the exent of the forward
  // trajectory.
  while (z>Z_MIN && z<fdc_z_wires[23]+1. && fdc_plane<24){
    // Bail if the momentum has dropped below some minimum
    if (fabs(S(state_q_over_p))>Q_OVER_P_MAX){
      if (DEBUG_LEVEL>2)
	{
...
jerror_t DTrackFitterKalmanSIMD::ExtrapolateForwardToOtherDetectors(){
  if (forward_traj.size()<2) return RESOURCE_UNAVAILABLE;
  //--------------------------------
  // First swim to Start counter and CDC/FDC layers
  //--------------------------------
  jerror_t error=ExtrapolateToInnerDetectors();

  //--------------------------------
  // Next swim to outer detectors...
  //--------------------------------
  if (error==NOERROR){
    return ExtrapolateToOuterDetectors(forward_traj[0].S);
  }
...
// Kalman engine for forward tracks, propagating from near the beam line to 
...
DTrackFitterKalmanSIMD::FindDoca(const DKalmanSIMDCDCHit_t *hit,
  ...
  if (do_brent==false){
    ...
    // In this case we swam the state vector to the position of the doca
    dy=S(state_y)-yw;
    dx=S(state_x)-xw; 
  }
...
void DTrackFitterKalmanSIMD::StepBack(double dedx,double newz,double z,
				      DMatrix5x1 &S0,DMatrix5x1 &S,
				      DMatrix5x5 &C){
  double dz=newz-z;
  DMatrix5x5 J;
  ...
  if (num_steps==0){
    ...
    double my_z=newz; 
    double my_dz=(dz>0.0?1.0:-1.)*mStepSizeZ;
    double dz3=dz-num_steps*my_dz;
    for (int m=0;m<num_steps;m++){
      z=my_z-my_dz;
...
void DTrackFitterKalmanSIMD::UpdateSandCMultiHit(const DKalmanForwardTrajectory_t &traj,
  ...
  for (unsigned int m=1;m<traj.num_hits;m++){
    ...
    if (my_fdchits[my_id]->status==good_hit){
      ...
      V(1,1)=my_fdchits[my_id]->vvar*fdc_anneal_factor;
