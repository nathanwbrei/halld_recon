...
void calibTimes() {
	...
	char old_offset_fname[256]; sprintf( old_offset_fname, "FCAL_timing_offsets_old.dat" );
		// pathname to file with current /FCAL/timing_offsets
	
	
	char new_offset_fname[256]; sprintf( new_offset_fname, "FCAL_timing_offsets_new.dat" );
		// pathname to file where the new /FCAL/timing_offsets will be written
	
	
	char root_fname[256]; sprintf( root_fname, "FCAL_Timing_shifts.root" );
	...
	if( CALIB_WITH_BEAM )
		h2 = (TH2I*)fIn->Get( "fcal_beam_dt_VS_chan" );
	else 
		h2 = (TH2I*)fIn->Get( "fcal_rf_dt_VS_chan" );
	...
	for( int ich = 0; ich < 2800; ich++ ) {
		...
		//h1->Rebin( 2 );
		//h1->GetXaxis()->SetRangeUser( -14., 14. );
		...
		fGaus->SetRange( loc_time - .75*loc_width, loc_time + 1.*loc_width );
		
		TFitResultPtr result = h1->Fit( "fGaus", "SR0Q" );
		...
		if( (Int_t)result == 0 ) {
			...
			if( reducedChi2 < 10. ) {
				...
				// try increasing the fit range:
				
				loc_time  = h1->GetBinCenter( h1->GetMaximumBin() );
				loc_width = 0.2;
				fGaus->SetRange( loc_time - 1.*loc_width, loc_time + 1.5*loc_width );
